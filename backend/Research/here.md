...
## How the first architecture happened

The first version of this architecture did not come out of nowhere. At that stage, I asked my IDE AI to research the Google Maps and Places documentation and suggest an implementation direction.

Based on that earlier research pass, the conclusion seemed to be that I could store the Google response for up to 30 days. That guidance shaped the first architecture and made it feel reasonable to keep more Google-derived data in the database for a limited time.

Later, I did a much deeper review of the documentation myself and found that the safer interpretation was different from that first recommendation.

What I ultimately found was much narrower:

- `placeId` can be stored permanently
- `lat/lng` can be stored only as temporary cached data
- the rest of the Google place data should not be kept as long-term database content
- names, addresses, and other details should be fetched from the Google API on demand

That difference between the first AI-assisted interpretation and the later deeper documentation review is what triggered this architecture change. So this refactor was not just about improving code structure; it was also about correcting the system to match the stricter understanding of what the docs actually allow.
...
That content must now be fetched on demand, rendered through Google map UI where appropriate, or replaced by app-owned fallback values.
